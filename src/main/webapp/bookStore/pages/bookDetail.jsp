...
<%
//    String memId = (String) session.getAttribute("memId"); // 세션 쓸거면
	String memId = request.getParameter("id");
	String pwd = request.getParameter("pwd");
	String dquery = "";
    if (memId != null && pwd != null) {
    	dquery = "&id=" + memId + "&pwd=" + pwd;
    } else {
    	dquery = "";
    }
...
if ("POST".equalsIgnoreCase(request.getMethod()) && request.getParameter("delete_review_id") != null) {
    ...
    try {
        ...
        if (isOwner) {
            ...
            if (result > 0) {
                response.sendRedirect("bookDetail.jsp?book_id=" + bookId + "&page=" + pageNum + dquery);
                return;
            }
        } else {
%>
...
%>
    <script>
        alert("주문이 완료되었습니다.");
        location.href = "./orderDelivery.jsp?order_id=<%= orderId %><%= dquery %>";
    </script>
<%
    } else {
%>
...
<%
    ...
    try {
        ...
        if(result > 0) {
            // PRG 패턴 적용!
            response.sendRedirect("bookDetail.jsp?book_id=" + bookId + dquery);
            return;
        }
    } catch(Exception e) {
%>
...
<body>
  ...
  <div id="wrapper">
    ...
    <div class="pagination">
        <% if (totalPage > 1) { %>
            <% if(pageNum > 1) { %>
                <a href="bookDetail.jsp?book_id=<%= bookId %>&page=<%= pageNum - 1 %><%= dquery %>">◀ 이전</a>
            <% } %>
            <% for(int i = 1; i <= totalPage; i++) {
                String link = "bookDetail.jsp?book_id=" + bookId + "&page=" + i + dquery;
            %>
                <% if(i == pageNum) { %>
            ...
            <% if(pageNum < totalPage) { %>
                <a href="bookDetail.jsp?book_id=<%= bookId %>&page=<%= pageNum + 1 %><%= dquery %>">다음 ▶</a>
            <% } %>
        <% } %>
    </div>
